...
export function OrdenesInactivas({handleBackToMain}) {
  ...
  const fetchOrdenesInactivas = async () => {
    try {
      const response = await axios.get("https://backenddelicremfinal.onrender.com/api/ordenesproduccion/inactivas");
  ...
  const handleActivateOrder = async (idOrden) => {
    try {
      await axios.patch(`https://backenddelicremfinal.onrender.com/api/ordenesproduccion/${idOrden}/estado`, {
        activo: true
      });
      Toast.fire({
        icon: 'success',
