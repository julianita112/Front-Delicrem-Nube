...
export function Insumos() {
  ...
  const fetchInsumos = async () => {
    try {
      const response = await axios.get("https://backenddelicremfinal.onrender.com/api/insumos");
  ...
  const handleDelete = async (insumo) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://backenddelicremfinal.onrender.com/api/insumos/${insumo.id_insumo}`);
        fetchInsumos();
  ...
  const handleSave = async () => {
    try {
      ...
      if (editMode) {
        await axios.put(`https://backenddelicremfinal.onrender.com/api/insumos/${selectedInsumo.id_insumo}`, selectedInsumo);
        setOpen(false);
        ...
        await axios.post("https://backenddelicremfinal.onrender.com/api/insumos", selectedInsumo);
        fetchInsumos();
        setOpen(false);
        const Toast = Swal.mixin({
          toast: true,
    ...
    if (result.isConfirmed) {
      try {
        if (!estado) { // Solo verificamos si intentamos desactivar el insumo   
          const fichasResponse = await axios.get(`https://backenddelicremfinal.onrender.com/api/fichastecnicas`);
        ...
        await axios.patch(`https://backenddelicremfinal.onrender.com/api/insumos/${insumo.id_insumo}/estado`, {
          estado: estado
        });
