...
export function Proveedores() {
  ...
  const fetchProveedores = async () => {
    try {
      const response = await axios.get("https://backenddelicremfinal.onrender.com/api/proveedores");
  ...
  const handleDelete = async (proveedor) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://backenddelicremfinal.onrender.com/api/proveedores/${proveedor.id_proveedor}`);
  ...
  const handleSave = async () => {
    ...
    if (validateForm()) {
      try {
        if (editMode) {
          await axios.put(`https://backenddelicremfinal.onrender.com/api/proveedores/${selectedProveedor.id_proveedor}`, selectedProveedor);
          Toast.fire({
            icon: 'success',
            title: 'El proveedor ha sido actualizado correctamente.'
          });
        } else {
          await axios.post("https://backenddelicremfinal.onrender.com/api/proveedores", selectedProveedor);
          Toast.fire({
            icon: 'success',
  ...
  const toggleActivo = async (id_proveedor, estado) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.patch(`https://backenddelicremfinal.onrender.com/api/proveedores/${id_proveedor}/estado`, { estado: !estado });
        fetchProveedores(); 
        Toast.fire({
          icon: 'success',
