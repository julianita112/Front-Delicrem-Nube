...
export function OrdenesProducidas({handleBackToMain}) {
  ...
  const fetchOrdenesProducidas = async () => {
    try {
      const response = await axios.get("https://backenddelicremfinal.onrender.com/api/ordenesproduccion/producidas");
  ...
  const handleCompleteOrder = async (idOrden) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.put(`https://backenddelicremfinal.onrender.com/api/ordenesproduccion/${idOrden}/estado`, {
          estado: 'completado',
        });
        Toast.fire({
          icon: 'success',
